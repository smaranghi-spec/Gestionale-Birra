...
class Degustazione(Base):
    ...
    # Legacy (vecchio formato semplificato)
    ...
    # Scheda Unionbirrai UBT (50 punti totali)
    # Esame Visivo (max 15)
    limpidezza = Column(Float, nullable=True)
    colore = Column(Float, nullable=True)
    schiuma = Column(Float, nullable=True)
    # Esame Olfattivo (max 15)
    intensita_olfattiva = Column(Float, nullable=True)
    finezza_olfattiva = Column(Float, nullable=True)
    complessita_olfattiva = Column(Float, nullable=True)
    # Esame Gusto-Olfattivo (max 15)
    corpo = Column(Float, nullable=True)
    equilibrio = Column(Float, nullable=True)
    persistenza_gusto_olfattiva = Column(Float, nullable=True)
    # Impressione Generale (max 5)
    impressione_generale = Column(Float, nullable=True)
    note = Column(Text, nullable=True)
